...
@Autonomous(name = "RedAudience", group = "Iterative Opmode")
//@Disabled
public class AutoAudienceRedEncoders extends OpMode {
    ...
    private static final String LABEL_SECOND_ELEMENT = "tape";
    ...
    public void loop() {
        switch (task) {
            ...
            case "back up to 3-layer thingy":
                if (checkEncoderDone()) {
                    encoderComplete();
                    encoderDrive(DRIVE_SPEED, -38, -38);
            ...
            case "turn to face thingy":
                if (checkEncoderDone()) {
                    encoderComplete();
                    encoderDrive(TURN_SPEED - 0.2, 23, -23);
                    task = "lift to drop";
                }
                break;
            ...
            case "SPEEDY TO PARK":
                if (checkEncoderDone()) {
                    encoderComplete();
                    encoderDrive(DRIVE_SPEED, 41, 41);
                    runtime.reset();
                    task = "strafe to square";
                }
                break;

            case "strafe to square":
                if (checkEncoderDone()) {
                    encoderComplete();
                    if (runtime.seconds() > 2) {
                        encoderStrafe(0.6, 18, -18);
                        task = "parallel";
                    }
                }
                break;
...
//                break;
    ...
    public boolean checkLiftDone() {
        return !(robot.lift.isBusy());
    }

    public void liftComplete() {
        robot.lift.setPower(0);
